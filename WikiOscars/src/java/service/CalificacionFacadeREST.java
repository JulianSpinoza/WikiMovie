...
public class CalificacionFacadeREST extends AbstractFacade<Calificacion> {
    ...
    private CalificacionPK getPrimaryKey(PathSegment pathSegment) {
         ...
         * URI path part is supposed to be in form of 'somePath;idCalificacion=idCalificacionValue;idPeliculafk=idPeliculafkValue'.
        ...
        if (idCalificacion != null && !idCalificacion.isEmpty()) {
            key.setIdCalificacion(new java.lang.Integer(idCalificacion.get(0)));
        }
        java.util.List<String> idPeliculafk = map.get("idPeliculafk");
        if (idPeliculafk != null && !idPeliculafk.isEmpty()) {
            key.setIdPeliculafk(new java.lang.Integer(idPeliculafk.get(0)));
        }
        return key;
    }
